...
DEST_DIR_NAME="/samba_0/badubko-q/Back_F/BAS/Pagos"
...
then
  echo "$0: No esta montado el directorio origen ${ORIGIN_DIR_NAME}"
  exit
fi

if [ ! -f "${DEST_MARK_NAME}" ]
then
  echo "$0: No esta montado el directorio destino ${DEST_DIR_NAME}"
  exit
fi
...
printf "          Comienzo Detalle: %s  Generado por:  %s \n" 	"${START_TIME}"  ${0} >${FILE_NAME_REP_DETALL}
printf "          File reducido  : %s \n" 	"${FILE_NAME_REP_REDUC_DIA}" 							>>${FILE_NAME_REP_DETALL}

# Inicializar archivo reducido
printf "          Comienzo: %s  Generado por: %s \n" 	"${START_TIME}"  ${0}   >> "${FILE_NAME_REP_REDUC_DIA}"
printf "          Archivo detalle: %s \n" 	"${FILE_NAME_REP_DETALL}" 				 >>${FILE_NAME_REP_REDUC_DIA}

# printf "          File   : %s \n" 	"${SUB_DIR_DETALL_DATE}/${FILE_NAME_REP_DETALL}"

# Ejecutar respaldo
rsync -r ${RUN_TYPE} -t -p -o -g -v --progress --delete -i -s ${ORIGIN_DIR_NAME} ${DEST_DIR_NAME}  >>${FILE_NAME_REP_DETALL}
